...
@register_scanner
class KNNConsensusScanner(BaseScanner):
    """Adapter: kNN-consensus detector (data-level, tabular) — catches label flips."""
    name = "Stats: kNN label consensus"
    ...
    def run(self, context: ScanContext) -> ScanResult:
        try:
            import pandas as pd
            from src.detectors.data_level import extract_xy, knn_consensus_scores, outlier_flags, top_indices
        ...
        if frac >= self.review_frac:
            return H.review(self, verdict=f"{int(fl.sum())} rows disagree with kNN neighbours", **det)
        return H.ok(self, verdict="labels consistent with neighbours", **det)
