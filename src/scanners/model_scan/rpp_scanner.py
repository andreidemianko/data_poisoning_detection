...
@register_scanner
class RPPScanner(BaseScanner):
    """Adapter: RPP prediction-stability detector (model-level, L-inf)."""
    name = "Model: RPP (prediction stability under noise)"
    ...
    def run(self, context: ScanContext) -> ScanResult:
        try:
            import pandas as pd
            from src.detectors.model_level import prepare_model, rpp_scores
        ...
        det = {"label_column": label_col, "n_rows": len(X), "flagged": int(fl.sum()),
               ...
               "note": "RPP measures confidence/stability, not maliciousness -> REVIEW"}
        if frac >= self.review_frac:
            return H.review(self, verdict=f"{int(fl.sum())} abnormally-stable rows", **det)
